...
为了包含外部的 Node 模块在你的 config.ts 中，在 `~/.noiragent` 文件夹运行 `npm install <module_name>` ，然后在 config.ts 中导入它们。

Continue 将使用 [esbuild](https://esbuild.github.io/) 打包你的 `config.ts` 和任何依赖到一个单独的 Javascript 文件。确切使用的配置可以在 [这里](https://github.com/noiragentdev/noiragent/blob/5c9874400e223bbc9786a8823614a2e501fbdaf7/extensions/vscode/src/ideProtocol.ts#L45-L52) 找到。
...
```json
{
  "extensionDependencies": ["noiragent.noiragent"]
}
```

或者复制 `~/.noiragent/type/core/index.d.ts` 到你的扩展仓库。
...
const noiragentExt = vscode.extensions.getExtension("noiragent.noiragent");

// get the API from the extension
const noiragentApi = noiragentExt?.exports;

// register your custom provider
noiragentApi?.registerCustomContextProvider(customProvider);
```
