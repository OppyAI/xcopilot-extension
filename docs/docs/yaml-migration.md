...
Create a `config.yaml` file in your Continue Global Directory (`~/.noiragent` on Mac, `%USERPROFILE%\.noiragent`) alongside your current `config.json` file. If a `config.yaml` file is present, it will be loaded instead of `config.json`.
